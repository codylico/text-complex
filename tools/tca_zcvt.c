...
int do_zsrtostr(FILE *in, FILE* out) {
  ...
  int ec = EXIT_SUCCESS;
  struct tcmplxA_zcvt* state = tcmplxA_zcvt_new(1, 32768, 1);
  if (state == NULL)
    return EXIT_FAILURE;
...
int do_strrtozs(FILE *in, FILE* out) {
  ...
  int ec = EXIT_SUCCESS;
  int done = 0;
  struct tcmplxA_zcvt* const state = tcmplxA_zcvt_new(256, 32768, 1);
  if (state == NULL)
    return EXIT_FAILURE;
  else while (ec == EXIT_SUCCESS) {
    if (feof(in))
  ...
  while (ec == EXIT_SUCCESS && !done) {
    size_t retval;
    int const res = tcmplxA_zcvt_delimrtozs(state, &retval, outbuf, 256);
    if (res < tcmplxA_Success) {
      ec = EXIT_FAILURE;
...
int main(int argc, char**argv) {
  ...
  /* */{
    ...
    if (ifilename == NULL || ofilename == NULL) {
      fputs("usage: tcmplx_access_zcvt [-d] (input) (output)\n", stderr);
      return EXIT_FAILURE;
    }
  }
  /* */{
